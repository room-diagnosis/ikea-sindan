...
function showResult() {
  ...
  const result = results[roomType][type];
  ...
  surveyButton.href = result.surveyUrl || "https://forms.gle/CMxeCYZWbndvMiWq8";
...
function setupShareButtons(result) {
  ...
  instaShareButton.onclick = async function () {
    const instagramText =
      "私の結果は " +
      result.title +
      "！\n\n" +
    ...
    try {
      ...
      alert("シェア文をコピーしました。Instagramで貼り付けて使ってね。");
      window.open("https://www.instagram.com/", "_blank");
    } catch (error) {
      alert("コピーに失敗しました。下の文を手動でコピーしてInstagramで使ってね。\n\n" + instagramText);
    }
  };
}
